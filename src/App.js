...
//   }
  ...
  for(let i = 0; i < bots.length; i++){ 
    ...
    if(bots[i].ballGrabbed){
      bots[i].ballInHand(p5, balls, i, bots, playerAlive, cam, collidedBalls)
    }
  ...
  for(let i = 0; i < balls.length; i++){
    balls[i].collideWall(p5, hei, thick, a, e, wid, len, bots, playerAlive, ballGrabbed, balls, grabbedBall);
    //run collision command
